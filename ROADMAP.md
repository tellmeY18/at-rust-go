...
### 6.1 `atrg routes`
- [ ] Implement by introspecting the `axum::Router` (Axum 0.8 supports walking routes via `Router::routes()` or via a custom registry).
...
### 6.3 `atrg build`
- [ ] Wraps `cargo build --release` with timing summary + binary path.
...
### 6.6 Documentation 📝
- [ ] Write `docs/getting-started.md` (the 5-minute quickstart, JSON only).
- [ ] Write `docs/oauth.md` (how OAuth flow works, what `client-metadata.json` needs in production, plus the two-token model: atrg session token vs. AT Protocol JWT).
- [ ] Write `docs/jetstream.md` (writing event handlers, error semantics, backpressure, ZSTD dictionary, ordering guarantees).
- [ ] Write `docs/xrpc.md` (registering procedures, JWT auth, `XrpcError` envelope, auth-required vs. anonymous).
- [ ] Write `docs/codegen.md` (full lexicon → working endpoint walkthrough using a custom namespace).
- [ ] Write `docs/testing.md` (using `atrg-testing` to mock the AT Protocol surface).
...
- [ ] Write `docs/rfcs/0000-template.md` and `docs/rfcs/README.md` describing the lightweight RFC process for non-trivial features.
...
- [ ] Run `cargo doc --workspace --no-deps` and ensure no missing-docs warnings.
...
- [ ] 🧪 `atrg new release-test && cd release-test && atrg dev` → JSON 200 at `/`.
...
- [ ] 🧪 An XRPC route works end-to-end and emits proper error envelopes for every failure mode.
- [ ] 🧪 `atrg generate` against a custom `com.example.todo.*` lexicon produces compilable Rust.
- [ ] 🧪 `examples/todo` (custom-namespace lexicon) runs end-to-end.
- [ ] 🧪 `atrg new social-demo --template social` works end-to-end and `cargo test` inside the scaffold passes using `atrg-testing` mocks.
- [ ] 🧪 `atrg routes`, `atrg migrate --status`, `atrg build`, `atrg generate` all work.
- [ ] 🧪 `/healthz` and `/readyz` respond as expected; `/readyz` includes Jetstream + identity-cache metrics.
- [ ] 🧪 DID/handle resolution cache shows >90% hit rate after a warmup period.
- [ ] 🧪 No `unwrap()` / `expect()` reachable in any library crate (audit via `grep` + `clippy::unwrap_used`).
- [ ] 🧪 `cargo doc --workspace --no-deps` is warning-free.
- [ ] 🧪 `cargo deny check` is green.
- [ ] 🔒 Security checklist:
  - [ ] No secrets in scaffold defaults usable in production (warn + refuse to bind non-localhost with `CHANGE_ME` secret).
  - [ ] Cookies `Secure + HttpOnly + SameSite=Lax` in non-dev mode.
  - [ ] No access tokens, refresh tokens, or JWTs in JSON bodies or logs.
  - [ ] CORS defaults to closed; preflight `OPTIONS` works for `/api/*` and `/auth/*`.
  - [ ] Security headers present in production env (`X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, `CSP`).
...
- ORM (write SQL).
- Multi-tenancy.
- Postgres support (SQLite only; Postgres tracked for v0.2).
...
- Jetstream cursor persistence across restarts (tracked for v0.2; the v0.1.0 consumer always resumes from "now").
- Distributed identity cache (single-process `moka` only; Redis-backed cache tracked for v0.2).
- **Bundled lexicons of any kind.** No `app.bsky.*`, no `com.atproto.*`, no helpers, no typed records, no feature flags. Developers bring their own lexicon `.json` files and validate them with `atproto-lexicon` themselves. A future convenience crate bundling popular lexicons may exist as an out-of-tree, community-managed package — it will never be part of this repository or the v0.1.0 release.
...
```
Phase 0 (workspace, +identity/codegen/testing crate skeletons)
    │
    ▼
Phase 1 (skeleton + JSON server + AppState{identity})
    │
    ▼
Phase 2 (atrg-identity → OAuth → JWT verify → token refresh) ──┐
    │                                                            │
    ▼                                                            │
Phase 3 (Jetstream: backpressure, ZSTD dict, metrics) ───────────┤
    │                                                            │
    ▼                                                            │
Phase 4 (XRPC + XrpcError envelope + atrg-codegen) ──────────────┤
    │                                                            │
    ▼                                                            ▼
