...
//!
//! Provides atomic counters that are updated by the consumer tasks and a
//! serializable snapshot type for exposing metrics via health endpoints.
...
/// Shared, atomic counters for the firehose consumer.
#[derive(Debug)]
...
impl MetricsCounter {
    ...
    pub fn snapshot(&self, queue_depth: u64) -> FirehoseMetrics {
        FirehoseMetrics {
...
mod tests {
    ...
    fn metrics_counter_increments() {
        ...
        counter.last_seq.store(42, Ordering::Relaxed);
        let snapshot = counter.snapshot(3);
        assert_eq!(snapshot.events_received, 5);
        assert_eq!(snapshot.errors, 1);
        assert_eq!(snapshot.last_seq, 42);
    ...
    #[test]
    fn last_seq_tracks_negative_values() {
        let counter = MetricsCounter::new();
        counter.last_seq.store(-1, Ordering::Relaxed);
        let snapshot = counter.snapshot(0);
